...
{
	setlinestyle(PS_SOLID, 4);	// 设置线条样式 4表示宽度
	setlinecolor(color); // 颜色
	line(plf->x, plf->y, prf->x, prf->y); // 连线
...
{
	setlinestyle(PS_DOT, 2);//虚线
	setlinecolor(MAGENTA);
	circle(p_center->x, p_center->y, Rof); //绘制圆
...
{
	pmidr->pointTurn(*p_center, delta_theta); //后轴中点绕转向中心点转动
	...
	heading_theta += delta_theta;//偏航角更新
}

void CarBase::carRotationStep()//单帧旋转
{
...
void CarBase::updateRinRout(const double& R)//����4���뾶
{
	Ror = R + car_width / 2.0;
	Rir = R - car_width / 2.0;
...
void CarBase::updateDriftRotInfo()//����Ư����ת��Ϣ
{
	pmid->Rp = 0.0;
	plf->Rp = R0;
	prf->Rp = R0;
...
void CarBase::updateDriftRotRevInfo(const Point& center)//����Ư����ת+��ת��Ϣ
{
	//���¹�ת����
	if (p_center)
	{
