...
{
	road0 = make_unique<RoadNormal>();//创建子类对象
...
/******************************************停车站******************************************/
StraightStation::StraightStation()//停车站
{
...
void StraightStation::showScene()//绘制
{
	BeginBatchDraw();
	cleardevice();
...
bool StraightStation::planning_process()//整个过程
{
	...
	delay(stop_time * 1000);//停车等待3秒
	uniformAccByTime(speedlimit, 2.0);//加速离开
	return true;
}
...
void StraightFollow::showScene()//绘制
{
	BeginBatchDraw();
	cleardevice();
...
bool StraightFollow::planning_process()//整个过程
{
	...
	if (dis <= 0.0 || delta_dis <= 0.0 || delta_speed_y > 0.0)//仅当初始间距大于目标间距且需要减速时
	{
		return false;
	}
...
/******************************************ͨ通过斑马线******************************************/
StraightCrosswalk::StraightCrosswalk()//ͨ通过斑马线
{
...
void StraightCrosswalk::showScene()//绘制
{
	BeginBatchDraw();
	cleardevice();
...
bool StraightCrosswalk::planning_process()//整个过程
{
	//进入斑马线场景，一律先减速
	double dis = car0->pmidf->y - road0->getDownLine();
	car0->a_y = pow(car0->speed_y, 2) / 2.0 / dis; //计算加速度
	...
	while (dis > 0.0)
	{
		...
		{
			if (dis <= 7.0)//ֹͣ停止线前让车辆完全停下，防止慢速挪动或倒车
			{
				car0->speed_y = 0.0;
				car0->a_y = 0.0;
				break;
	...
	while (car0->pmidr->y > 0.0)
	{
		if (car0->speed_y > speedlimit)//没有达到道路限速
		{
			car0->a_y = -0.05;//加速
		}
		else
